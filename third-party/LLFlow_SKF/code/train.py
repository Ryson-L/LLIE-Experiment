    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                epoch_like = current_step // opt['train']['val_freq']
                save_dir = opt['path']['val_images']
                util.mkdir(save_dir)
                ...
                for val_data in val_loader:
                    ...
                    rlt_img = (cropped_sr_img_adjust * 255).astype(np.uint8)  
                    # save enhanced photos
                    if rank <= 0:
                        base_name = img_name  # 这里 img_name 类似 '0186'
                        img_name_epoch = 'Epoch{:03d}_{}_Pred.png'.format(epoch_like, base_name)
                        save_path = os.path.join(save_dir, img_name_epoch)
                ...
                metrics_name = 'metrics_Epoch{:03d}.txt'.format(epoch_like)
