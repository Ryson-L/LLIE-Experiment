...
from models.loss import CharbonnierLoss, CharbonnierLoss2, hist_loss, portrait_l1_loss
...
class VideoBaseModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        if self.seg_map is not None and train_opt.get('use_portrait_loss', False):
            # 统一成 int / float，避免 YAML 读成字符串
            person_id = int(train_opt.get('portrait_class_id', 36))
            weight = float(train_opt.get('portrait_loss_weight', 0.0))

            if weight > 0:
                l_portrait_raw = portrait_l1_loss(
                    self.seg_map,      # seg_pred
                    self.fake_H,       # enhanced
                    self.real_H,       # GT
                    person_id=person_id
                )
                # 有可能当前 batch 没有人像像素 -> 返回 0
                if l_portrait_raw is not None and l_portrait_raw.item() > 0:
                    l_portrait = weight * l_portrait_raw

        if l_portrait is not None:
            l_final = l_final + l_portrait
