...
class MaskedSSIMLoss(nn.Module):
    ...
    def forward(self, img1, img2, mask):
        ...
        # 调用核心计算
        return self._masked_ssim(img1, img2, window, self.window_size, channel, mask)

    def _masked_ssim(self, img1, img2, window, window_size, channel, mask):
        # 1. 计算局部均值 (Mean)
        mu1 = F.conv2d(img1, window, padding=window_size // 2, groups=channel)
        mu2 = F.conv2d(img2, window, padding=window_size // 2, groups=channel)
